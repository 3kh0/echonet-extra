<template>
  <main class="p-6 min-h-screen">
    <h1 class="text-3xl font-bold mb-6 text-center">Crypto Wallets</h1>
    <div v-for="wallet in wallets" :key="wallet.name" class="wallet bg-gray-800 p-4 rounded-lg shadow-md mb-6">
      ...
      <p class="text-gray-300 mb-4 break-words">Address: <span class="font-mono">{{ wallet.address }}</span></p>
      ...
      <div class="flex space-x-4">
        <button @click="copyAddress(wallet.address)" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
          <img src="/svg/copy.svg" alt="Copy address" class="w-6 h-6">
        </button>
        <button @click="generateQRCode(wallet.address)" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
          <img src="/svg/qr.svg" alt="Get QR Code" class="w-6 h-6">
        </button>
        <button @click="viewOnBlockExplorer(wallet.symbol, wallet.address)" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">
...
const viewOnBlockExplorer = (symbol, address) => {
  ...
  switch (symbol) {
    ...
    case 'btc':
      ...
      break;
    case 'ltc':
      explorerUrl = `https://explorer.litecoin.net/address/${address}`;
      break;
    case 'eth':
    ...
    case 'sol':
      ...
      break;
    case 'bch':
      explorerUrl = `https://explorer.bitcoin.com/bch/address/${address}`;
      break;
    default:
